...
def count_metrics_for_task(es_obj, index_name, query_text, retrieved_documents, relevant_documents_lookup):
    true_relevant = 0
...
def evaluate_search(es_obj, results_path):
    ...
    results_file.write('Raw index MAP@20: {}. Stemmed index MAP@20: {}\n'.format(map_k_raw, map_k_stemmed))
    results_file.write('Task    p@20(raw/stemmed)   r@20(raw/stemmed)   r-precision(raw/stemmed)\n')
    for task in raw_results_dict:
        results_file.write('{}  {}/{}   {}/{}   {}/{}\n'.format(task, raw_results_dict[task][0],
                                                                stemmed_results_dict[task][0], raw_results_dict[task][1],
                                                                stemmed_results_dict[task][1], raw_results_dict[task][2],
                                                                stemmed_results_dict[task][2]))
    results_file.close()
